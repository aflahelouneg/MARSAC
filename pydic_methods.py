...
def identify_neighbours(center, points):
    values = []
    dtype = [('dist', float), ('coords', object)]
    for point in points:
        point = point.tolist()
        dist_eucl = (np.sqrt((point[0] - center[0])**2 + (point[1] - center[1])**2))
        coords = point
        values.append((dist_eucl, coords))

    dist = np.array(values, dtype=dtype)
    dist_sorted = np.sort(dist, order='dist') # sort the ascending dist list according to the distances
                                              # between each point and the refpoint
    neighbours = [np.array(dist_sorted[i][1]) for i in list(range(4))]

    return neighbours

def get_neighbours(refpoint, points):
    return identify_neighbours(refpoint, points)

def near(p1, p2, dist):
    # print(p1, p2)
    if np.abs(p2 - p1) <= dist:
        return True
    else:
        return False


def compute_refpoint_disp(refpoint, mean_x, mean_y, neighbours, nodes_data):
    ''' In this function, the displacement of the refpoint is calculated
    through interpolation from the 4 neighbour points '''

    # print(mean_x, mean_y)
    X = [neighbours[i][0]-mean_x for i,n in enumerate(neighbours)]
    Y = [-neighbours[i][1]+mean_y for i,n in enumerate(neighbours)]

    # print('X, Y', X, Y)
    # input('pause')

    EPS_pos = 0.1 # tolerance gap used to identify node position
    ...
    for j,neighbour in enumerate(neighbours):
        for node in nodes_data[1:]:
            # print('nodes', node[3], node[4])
            if near(X[j], node[3], EPS_pos) and near(Y[j], node[4], EPS_pos):
                Zx.append(node[7])
                Zy.append(node[8])
    ...
    refpoint[0] = refpoint[0] - mean_x
    refpoint[1] = -refpoint[1] + mean_y
    # print('refpoint', refpoint)
    # input('pause')
    # print('X, Y', X, Y)
    # input('pause')

    interpolation_X = interp2d(X,Y,Zx, kind='linear', copy=True,
                                bounds_error=False, fill_value=None)

    interpolation_Y = interp2d(X,Y,Zy, kind='linear', copy=True,
                                bounds_error=False, fill_value=None)

    refpoint_Dx = interpolation_X(refpoint[0], refpoint[1])
    refpoint_Dy = interpolation_Y(refpoint[0], refpoint[1])


    # print(refpoint_Dx, refpoint_Dy)
    # input('pause')
    return [refpoint_Dx, refpoint_Dy]

def init(image_pattern, win_size_px, grid_size_px, result_file,
                area_of_intersest=None, *args, **kwargs):
...
sequence of images. The displacements are computed and a result file is written
 ...
 - the refpoint used to compare displacements from Correlation process
                 # and the reality (computed directly from the screen)
     """
     ...
     # neighbours = identify_neighbours(refpoint, points)
     # print(neighbours)
     # input('pause')
...
"""
     ...
     print("Saving tables and displacement fields figures")
